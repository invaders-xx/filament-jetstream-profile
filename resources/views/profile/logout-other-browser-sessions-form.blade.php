<x-filament::card :header="__('Browser Sessions')">
    ...
    <div class="flex items-center mt-5">
        ...
        <x-filament-jetstream-profile::action-message class="ml-3" on="loggedOut">
            {{ __('Done.') }}
        </x-filament-jetstream-profile::action-message>
    </div>

    <!-- Log Out Other Devices Confirmation Modal -->
    <x-filament-jetstream-profile::dialog-modal wire:model="confirmingLogout">
        <x-slot name="title">
            {{ __('Log Out Other Browser Sessions') }}
        </x-slot>
        ...
        <x-slot name="content">
            ...
            <div class="mt-4" x-data="{}"
                ...
                <x-filament-jetstream-profile::input-error for="password" class="mt-2"/>
            </div>
        </x-slot>

        <x-slot name="footer">
            <x-filament-jetstream-profile::secondary-button wire:click="$toggle('confirmingLogout')"
                                                            wire:loading.attr="disabled">
                {{ __('Cancel') }}
            </x-filament-jetstream-profile::secondary-button>

            <x-filament::button class="ml-3"
